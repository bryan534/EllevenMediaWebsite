<script lang="ts">
	...
	onMount(() => {
		...
		currentContainer?.addEventListener("click", handleLinkClick);
...
>
	...
	>
		...
		>
			...
			<span
				class={cn(
					"ml-1 text-sm font-medium text-foreground transition-[color] duration-400 ease-[cubic-bezier(0.625,0.05,0,1)] group-hover:text-accent",
					classes?.toggleLabel,
				)}
			>
				Menu
			...
			{#each menuGroups as group (group.title)}
				...
				>
					<h3
						...
						class={cn(
							"font-sans text-xs font-medium tracking-wider text-foreground-muted/50 uppercase",
							classes?.groupTitle,
						)}
					>
						{group.title}
